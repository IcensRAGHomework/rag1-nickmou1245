...
def generate_hw03(question2, question3):
    ...
    response = history_handler.invoke(
        {"holiday_list": generate_hw02(question2),
         "question": "請根據問題列出台灣的紀念日，以 JSON 格式輸出 add : 這是一個布林值，表示是否需要將節日新增到節日清單中。根據問題判斷該節日是否存在於清單中，如果不存在，則為 true；否則為 false。 reason : 描述為什麼需要或不需要新增節日，具體說明是否該節日已經存在於清單中，以及當前清單的中文內容。"},
        config={"configurable": {"session_id": "holidays"}},
    )
